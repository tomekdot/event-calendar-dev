...
namespace FetchInternal {
    ...
    string g_QueuedCalendarUrl      = "";
    bool   g_QueuedCalendarFlushScheduled = false;
}
...
namespace Fetch {
    ...
    // -------------------------------------------------------------------------
    // Type Definitions & Core Tracking State
    // -------------------------------------------------------------------------
    ...
    // L1 Cache: Raw Response Body (String)
    ...
    // Request Coalescing (Preventing duplicate network calls)
     ...
     * Attempts storage folder first, falls back to user game folder.
     */
    string GetCacheDir() {
        string dir = "";

        try { 
            dir = IO::FromStorageFolder(FetchInternal::FILE_CACHE_DIR); 
        } catch { 
            dir = ""; 
        }
        ...
        if (dir.Length == 0) {
            try { 
                dir = IO::FromUserGameFolder(FetchInternal::FILE_CACHE_DIR); 
            } catch { 
                dir = ""; 
            }
        }
    ...
    /*
     * Persists the current month payload to disk.
     * Allows the calendar to recover instantly after a game restart.
     */
    void SaveToFileCache(int year, int month, const array<EventItem@>@ events) {
        ...
        meta["cachedAtStr"] = tostring(int64(Time::Stamp) * 1000); // Prevent precision loss
        meta["events"]      = arr;
        ...
        if (S_EnableDebug) {
            trace("[Moon] Saved " + tostring(events.Length) + " events to file cache: " + filepath);
        }
    }

    /*
     * Loads a previously saved month snapshot from disk.
     * If ignoreExpiration is true, we accept stale data (useful for offline mode).
     */
    bool LoadFromFileCache(int year, int month, array<EventItem@>@ &out events, bool ignoreExpiration = false) {
        ...
        if (root.HasKey("cachedAtStr")) {
             ...
             if (val.GetType() == Json::Type::String) {
                 ...
                 // Convert through double to avoid 32-bit truncation in some environments
        ...
        bool invalidTimestamp = (
            cachedAtMs == 0 || 
            cachedAtMs < 1577836800000 || // Before 2020-01-01 epoch
            (cachedAtMs > currentMs && cachedAtMs - currentMs > maxFutureMs)
        );

        // Debug Expiration Logging
        if (S_EnableDebug) {
            trace("[Moon] LoadFromFileCache: cachedAtMs=" + tostring(cachedAtMs) + " currentMs=" + tostring(currentMs));
            if (cachedAtMs > currentMs) {
                trace("[Moon] Future timestamp detected. Diff: " + tostring(cachedAtMs - currentMs) + "ms");
            }
        }
        
        // Handle Invalid / Expired logic
        if (!ignoreExpiration && invalidTimestamp) { 
        ...
        // Ensure events exist
        if (!root.HasKey("events")) return false;
        ...
        if (S_EnableDebug) {
            trace("[Moon] Loaded " + tostring(events.Length) + " events from file cache");
        }
    ...
    // =========================================================================
    // L1 Cache Operations (Raw Network Responses)
    // =========================================================================

    /*
     * Gets raw response body from memory cache. Returns true ONLY for fresh entries.
     */
    ...
    // =========================================================================
    // L2 Cache Operations (Parsed Event Objects)
    // =========================================================================

    /*
     * Gets parsed events from memory cache. Returns true ONLY for fresh entries.
     * If useStale=true, outEvents may still be populated to provide instant UI fallback.
     */
    bool GetCachedParsedEvents(const string &in url, array<EventItem@>@ &out outEvents, bool useStale = false) {
        ...
        try { 
            cachedTimestamp = int64(g_ParsedCacheTs[url]); 
        } catch {
        ...
        if (nowMs - cachedTimestamp > maxAge) {
            // Hard delete if it's way past expiration (e.g. 12x normal TTL)
        ...
        // Return true only if cache is completely fresh
    ...
    void SetCachedParsedEvents(const string &in url, const array<EventItem@>@ events) {
        ...
        try {
            ...
            if (S_EnableDebug) {
                trace("[Moon] L2 cache stored: " + url + " (count=" + tostring(stored.Length) + ")");
            }
        } catch {
    ...
    /*
     * Rejects bursts of identical navigation requests before spinning up a coroutine.
     */
    ...
    void _QueueCalendarFetch(const string &in url) {
        if (url.Length == 0) return;
        FetchInternal::g_QueuedCalendarUrl = url;
        if (!FetchInternal::g_QueuedCalendarFlushScheduled) {
            FetchInternal::g_QueuedCalendarFlushScheduled = true;
            startnew(_DelayedFlushQueuedCalendarFetch);
        }
    }

    void _DelayedFlushQueuedCalendarFetch() {
        while (FetchInternal::g_QueuedCalendarUrl.Length > 0) {
            sleep(100);
            _FlushQueuedCalendarFetch();
        }
        FetchInternal::g_QueuedCalendarFlushScheduled = false;
    }

    void _FlushQueuedCalendarFetch() {
        if (FetchInternal::g_QueuedCalendarUrl.Length == 0) return;

        string queuedUrl = FetchInternal::g_QueuedCalendarUrl;
        string currentUrl = GetCurrentCalendarUrl();

        if (currentUrl.Length > 0) {
            queuedUrl = currentUrl;
        }

        if (queuedUrl.Length == 0) return;
        if (g_PendingStarts.Exists(queuedUrl) || g_InFlightRequests.Exists(queuedUrl)) return;
        if (ShouldDebounce(queuedUrl)) return;

        FetchInternal::g_QueuedCalendarUrl = "";
        _InitAndStartFetch(queuedUrl, false, null);
    }
    ...
    bool _TryApplyMemoryCache(const string &in url, bool allowStale = true) {
        ...
        // Fetch fresh data in the background if what we showed was stale
        if (!isFresh) {
    ...
    bool _TryApplyFileCacheForCurrentMonth(bool ignoreExpiration = false) {
        if (g_UIState.CalYear == 0 || g_UIState.CalMonth == 0) return false;
    ...
    void _HandleFetchFailure(const string &in url, int httpCode, bool isInitialFetch) {
        ...
        // Schedule an automatic background retry
        if (!g_PendingStarts.Exists(url)) {
    ...
    void FetchForCalendarView() {
        ...
        // 1. Memory cache is the fastest path for month switching.
        if (_TryApplyMemoryCache(url, true)) {
            return;
        }

        // 2. File cache keeps the UI responsive while network refreshes later.
        if (_TryApplyFileCacheForCurrentMonth(false)) {
            StartFetchCoroutine(url, false);
            return;
        }

        // 3. No cache available. Clear view and fetch async.
    ...
    void AutoRetryFailedFetches() {
        ...
        // Only retry if we have SOME failures but haven't completely exceeded the failure limit
    ...
    /*
     * Performs HTTP request with capped exponential backoff logic.
     */
    bool FetchWithAutoRetry(const string &in url, string &out outBody) {
        ...
        while (attempt <= FetchInternal::MAX_RETRY_ATTEMPTS) {
            ...
            // Handle massive consecutive failure cooldown
            if (FetchInternal::g_ConsecutiveFailures >= FetchInternal::MAX_CONSECUTIVE_FAILURES) {
                int64 cooldownMs       = 30 * 1000; // 30s cooldown
            ...
            if (lastCode == 200) {
                // Success - Reset failure trackers
                FetchInternal::g_ConsecutiveFailures = 0;
                outBody = body;
                return true;
            }
    ...
    // =========================================================================
    // Core Background Worker (Coroutine)
    // =========================================================================
    ...
    void FetchCoroutine(ref@ args_ref) {
        ...
        bool isCurrentCalendarRequest = isInitialFetch || url == GetCurrentCalendarUrl();
        if (!isCurrentCalendarRequest) {
        ...
        if (cachedParsed !is null && cachedParsed.Length > 0) {
            ...
            // Schedule background refresh if the data we presented was stale
        ...
        if (httpCode != 200) {
            if (!isInitialFetch && url != GetCurrentCalendarUrl()) {
                _CleanupRequestState(url);
                _FlushQueuedCalendarFetch();
                return;
            }
    ...
    void _NotifyWaiters(const string &in url) {
        ...
        for (uint i = 0; i < waitList.Length; i++) {
            ...
            if (g_FetchHandlers.Exists(key)) {
                ...
                if (h !is null) {
                    try { 
                        h(@g_Events, waiterInitial); 
                    } catch { 
                        warn("[Moon] Waiter handler exception"); 
                    }
                }
    ...
    // =========================================================================
    // API Response Processing & Formatting
    // =========================================================================

    /*
     * Parses raw JSON response into EventItem objects, skipping invalid entries
    ...
    /*
     * Replaces the visible event list, optionally persists it, 
     * then rebuilds the per-day lookup cache.
     */
    void UpdateEventsAndCache(const array<EventItem@>@ local_events, bool persistToFile = true) {
        ...
        // Debug dump of first few events
        ...
        if (persistToFile && g_UIState.CalYear != 0 && g_UIState.CalMonth != 0 && g_Events.Length > 0) {
            try { 
                SaveToFileCache(g_UIState.CalYear, g_UIState.CalMonth, g_Events); 
            } catch {}
        }
    ...
    void HandleCalendarFetchSuccess() {
        ...
        if (S_EnableDebug) {
            trace("[Moon] Calendar fetch: " + g_UIState.CalYear + "-" + g_UIState.CalMonth + " (" + g_Events.Length + " events)");
        }
    }
}
