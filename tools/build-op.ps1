...
# Create a dist folder for the final .op file
$distDir = Join-Path $root 'dist'
if (-not (Test-Path -LiteralPath $distDir)) { New-Item -ItemType Directory -Path $distDir -Force | Out-Null }

$zipPathVersioned = Join-Path $distDir $versionedOutput
if (Test-Path $zipPathVersioned) { Remove-Item -LiteralPath $zipPathVersioned -Force }
[System.IO.Compression.ZipFile]::CreateFromDirectory($stage, $zipPathVersioned)
Write-Log "Created $zipPathVersioned"

# Cleanup
Remove-Item -LiteralPath $stage -Recurse -Force
Write-Log "Build complete. You can find the .op file in the 'dist' folder ready to be shared."
