...
# Prefer the plugin root (parent of tools) when this script lives in a 'tools' folder
$scriptDir = (Get-Item -LiteralPath $scriptPath).Directory.FullName
if ((Split-Path $scriptDir -Leaf) -ieq 'tools') {
...
try {
    ...
    if (Test-Path $zipPathVersioned) {
        Remove-Item -LiteralPath $zipPathVersioned -Force -ErrorAction Stop
    }
    Copy-Item -LiteralPath $zipPathSimple -Destination $zipPathVersioned -Force -ErrorAction Stop
    Write-Log "Copied versioned archive to $zipPathVersioned"
} catch {
    Write-Log "Could not write versioned archive to $pluginsAncestor (file may be locked): $_" -IsError
    $altName = "$safeName-v$version-$(Get-Date -Format yyyyMMddHHmmss).op"
    $altPath = Join-Path $pluginsAncestor $altName
    try {
        Copy-Item -LiteralPath $zipPathSimple -Destination $altPath -Force -ErrorAction Stop
        Write-Log "Copied versioned archive to $altPath (fallback due to lock)"
    } catch {
        Write-Log "Failed to copy fallback archive as well: $_" -IsError
    }
}
